...
- [app/lib/ai/generate.ts](app/lib/ai/generate.ts) — `runGeneration()`: demand-driven, **per-user-worst-case**. Asks `sat.min_active_user_unseen()` for the smallest unseen-enabled-questions count across active students (those with ≥ 1 attempt). No-op while that value is null (no active students) or `>= BUFFER_TARGET` (100); otherwise generates a bounded batch for the thinnest `(section, skill)`, runs the quality gate (zod → self-verify → dedup), and inserts survivors via the service-role client.

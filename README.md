...
`/api/admin/generate-questions` every 6 hours (deployed only). The endpoint checks
pool depth per `(section, skill)`, generates a small batch for the most-depleted
skill(s), runs the quality gate, and inserts survivors. It returns a JSON summary:
...
- vercel.json                        Vercel Cron — calls generation endpoint every 6 hours

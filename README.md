...
`/api/admin/generate-questions` **every hour** (deployed only). Generation is
**demand-driven**: the endpoint counts never-attempted questions (enabled pool
questions with no `served_questions` row) and only generates while that buffer
is below 100 — otherwise the run is a fast no-op. When it does generate, it
picks the thinnest `(section, skill)` for topic variety, runs the quality gate
(zod → self-verify → dedup), and inserts survivors; the hourly runs refill the
...
- vercel.json                        Vercel Cron — calls generation endpoint hourly
